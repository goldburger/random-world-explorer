...
var terrainVertices = new Array(terrainCount * 4);
...
function initTerrain() {
    ...
    for(var i=0; i<VERTEX_COUNT; i++){
        for(var j=0; j<VERTEX_COUNT; j++){
			terrainVertices[vertexPointer*4] = j/(VERTEX_COUNT - 1.0) * SIZE;
            height = simplexTerrain.noise(j,i);
            terrainVertices[vertexPointer*4+1] = height;	// y = height
            terrainVertices[vertexPointer*4+2] = i/(VERTEX_COUNT - 1.0) * SIZE;
            terrainVertices[vertexPointer*4+3] = 1;
    ...
    terrainVertexPositionBuffer.itemSize = 4;
